...
def save_kp_and_pc_in_pcd(pc, kp, output_dir, save=True, name=""):
    ...
    pcd = o3d.geometry.TriangleMesh.create_sphere(radius=0.005)
    pcd.translate(pc[0])
    pcd.paint_uniform_color(palette_PC[7])

    ''' Add points in the original point cloud'''
    for i in range(len(pc)):
        point = o3d.geometry.TriangleMesh.create_sphere(radius=0.005) # 0.005
    ...
    for i in range(0, len(kp)):
        point = o3d.geometry.TriangleMesh.create_sphere(radius=0.012)
        point.translate(kp[i])
        color_idx = i % len(palette)
        point.paint_uniform_color(palette[color_idx])
        pcd += point
